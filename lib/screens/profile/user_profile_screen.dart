...
class UserProfileScreen extends StatelessWidget {
                ...
                child: Column(
                  children: [
                    _buildSection(context, [
                      _buildOption(context, Icons.edit_outlined,
                          'Chỉnh sửa thông tin', '/add-edit-profile', scheme),
                      _buildOption(context, Icons.lock_outline,
                          'Đổi mật khẩu', '/change-password', scheme),
                      _buildSwitchOption(context, Icons.fingerprint_outlined,
                          'Xác thực sinh trắc học', scheme),
                    ]),
                    const SizedBox(height: 16),
                    _buildSection(context, [
  ...
  Widget _buildOption(BuildContext context, IconData icon, String title,
      String route, ColorScheme scheme) {
    return ListTile(
      leading: Container(
        width: 36,
        height: 36,
  ...
  Widget _buildSwitchOption(
      BuildContext context, IconData icon, String title, ColorScheme scheme) {
    return ListTile(
      leading: Container(
        width: 36,
        height: 36,
  ...
  Widget _buildLogoutTile(BuildContext context) {
    return ListTile(
      ...
      onTap: () async {
        // Reset provider và xóa token
        context.read<FinanceProvider>().reset();
